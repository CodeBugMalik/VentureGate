...
export default function PrivacyFlowViz({ status }: { status: VerifyStatus }) {
  const steps = [
    { 
      ...
      sublabel: 'Financial data stays on device', 
      icon: <Lock size={24} />, 
  ...
  return (
    <div className="card privacy-viz-card">
      ...
      <div className="privacy-notice">
        ...
        <span><strong>Your actual net worth and income are never sent to the network.</strong> The Midnight blockchain only records a cryptographic proof that you satisfy the eligibility threshold — nothing more.</span>
      </div>
    </div>
  );
}
